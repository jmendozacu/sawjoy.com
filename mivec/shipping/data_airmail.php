...
$sql = "SELECT 
		A.id,A.carrier_id,A.country_id,A.quote,A.tracking_no,A.updated_at,B.country as country_name,B.iso ,C.carrier_id,carrier_name
	FROM " . __TABLE_AIRMAIL_DATA__ . " A LEFT JOIN " . __TABLE_COUNTRY__ . " B ON(A.country_id=B.id)
		LEFT JOIN " . __TABLE_CARRIER__ . " C ON (A.carrier_id = C.carrier_id)
	WHERE 1
	"
;

if (!empty($_carrier)) $sql .= " AND A.carrier_id=" . $_carrier;
if (!empty($_country)) $sql .= " AND A.country_id=" . $_country;
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="list_row" style="background:#e8e8e8">
                    ...
                    <tr>
                        <td><table width="100%" border="0" cellspacing="1" cellpadding="0">
                                <tr>
                                    ...
                                    <td width="88%"><?php echo formSelect('carrier' , $_globalData['carrier_airmail'] , $_carrier)?></td>
                                </tr>
                                <tr>
    ...
    <?php if ($row):?>
        ...
        <tr>
            ...
            <td width="23%">Country</td>
            <td width="6%">ISO</td>
            <td width="13%">Quote</td>
        ...
        foreach ($frs as $rs) :?>
            <tr class="data">
                ...
                <td><?php echo $_globalData['currency'].$rs['price']['qupte']?></td>
                <td><?php echo $_globalData['currency'].$rs['price']['tracking_no']?></td>
                ...
                <td>
                    ...
                    <a href="javascript:;" onclick="deleteData(<?php echo $rs['carrier_id'];?>)" title="Delete">Delete</a>
                </td>
            </tr>
            <?php $i++;endforeach;?>
        <tr>
