...
OnePage.prototype = {
        ...
        $ji(document).on('change', section + ' .iwd_opc_select', function (e) {
            ...
            if ($ji(this).attr('name') && $ji(this).attr('name').indexOf('country_id') !== -1) {
                ...
                if (typeof(optionalZipCountries) !== 'undefined'
                    && $ji(this).val()
                    && optionalZipCountries.indexOf($ji(this).val()) !== -1) {
                    var placeholder = $ji(this).closest('form').find('input[name*="postcode"]').attr('placeholder').replace('*', '').trim();
                    $ji(this).closest('form').find('input[name*="postcode"]').attr('placeholder', placeholder);
                } else {
                    var placeholder = $ji(this).closest('form').find('input[name*="postcode"]').attr('placeholder').replace('*', '').trim();
                    $ji(this).closest('form').find('input[name*="postcode"]').attr('placeholder', placeholder + '\xa0*');
                }
            }
    ...
    initColumns: function () {
        ...
        $ji(document).on('focus', _this.sectionContainer + ' .iwd_opc_column_content', function () {
            ...
            $ji(_this.sectionContainer + ' .iwd_opc_column_content').each(function (e) {
                ...
                $ji(this).find('form.iwd_opc_form').trigger('iwdOpcValidateColumnBlur');
            });
        });
    },
    initConfig: function () {
    ...
    columnIsValid: function (column) {
        return column.find('.iwd_opc_form.iwd_opc_form_invalid:visible').length === 0
    ...
    isCheckoutValid: function () {
        ...
        if (!this.config.isVirtual) {
            if ($ji(Singleton.get(BillingAddress).sectionContainer + ' .iwd_opc_checkbox[name="billing[use_for_shipping]"]').is(':checked')) {
    ...
    changeField: function (element, e) {
        ...
        if (_this.saveDelay === -1) {
            var form = element.closest('form.iwd_opc_form');
            var column = form.closest('.iwd_opc_column_content');
            if (!column.hasClass('focused')) {
                ...
                $ji(form).off('iwdOpcValidateColumnBlur');
                $ji(form).on('iwdOpcValidateColumnBlur', function () {
    ...
    parseSuccessResult: function (result) {
        ...
        if (typeof(result.centinel_frame) !== 'undefined' && result.centinel_frame) {
            this.showPopup(result.centinel_frame, false);
            $ji('#centinel_authenticate_block').show();
        }
    ...
    decorateSelect: function (select) {
        ...
        if (select.find('option').length && select.is(':visible')) {
            ...
            } else if (select.find('option:selected').html().trim().length || (select.find('option:selected').attr('data-image') && select.find('option:selected').attr('data-image').length)) {
    ...
    appendScrollBar: function (container) {
        if (container.children().length > 5 || container.actualScrollHeight() > parseInt(container.css('max-height').replace('px', ''))) {
            container.scrollbar();
        }
    }
};
